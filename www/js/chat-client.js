$(function() {
    ...
    function inputName() {
      var imgN = Math.floor(Math.random()*30); // 随机分配头像
      if($('#name').val().trim()!=='')
          socket.emit('login', { 
            name: $('#name').val(),
            img: 'image/IMG_' + imgN + '.jpg'
          });  // 触发登录事件
      return false; 
    }
    ...
    socket.on('shake', (user)=> { 
      var data = new Date().toTimeString().substr(0, 8);
      $('#messages').append(`<p class='system'><span>${data}</span><br /><span>${user.name}有话要说</span></p>`);
      shake();
      // 滚动条总是在最底部
      $('#messages').scrollTop($('#messages')[0].scrollHeight);
    });
